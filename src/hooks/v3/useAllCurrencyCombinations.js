...
import { ADDITIONAL_BASES, BASES_TO_CHECK_TRADES_AGAINST } from 'config/constants/v3/routing'
...
export const useAllCurrencyCombinations = (currencyA, currencyB) => {
  ...
  const basePairs = useMemo(() => bases.flatMap((base) => bases.map((otherBase) => [base, otherBase])).filter(([t0, t1]) => !t0.equals(t1)), [bases])
